...
describe("monthlyRecurringCents", () => {
  ...
  it("counts only active, unfrozen tenants", () => {
    const rows = [
      tenant({ planType: "pro", subscriptionStatus: "active" }), // 12000
      tenant({ planType: "starter", subscriptionStatus: "active" }), // 8000
    ...
    expect(monthlyRecurringCents(rows)).toBe(20000); // 12000 + 8000
  });
  ...
  it("separates the trial pipeline from committed revenue", () => {
    ...
    expect(monthlyRecurringCents(rows)).toBe(12000);
    expect(trialPipelineCents(rows)).toBe(8000);
  });
});
