...
/**
 * Whether the grid hides the hours nothing is booked in.
 *
 * ---------------------------------------------------------------------------
 * **The grid's default height is the shop's opening hours plus an hour each
 * side**, which answers "where could I still put someone". An owner who opens
 * at eight and whose first client this week is at eleven scrolls past three
 * empty hours to find the first card — every day, on the phone the product is
 * opened on. Cropped, the grid runs from the first booking's hour to the last
 * one's, with no padding, and a week with nothing booked falls back to the
 * opening hours rather than to an empty strip — see `gridBounds`.
 *
 * A preference beside the density, kept the same way and for the same reasons:
 * the owner's to choose, remembered on this device, and read as an external
 * store so the server's render and the browser's agree through hydration.
 * ---------------------------------------------------------------------------
 */
export const FIT_HOURS_STORAGE_KEY = "bazman.calendar-fit-hours";

/** The stored choice; off wherever storage is missing, refused or edited. */
export function readStoredFitHours(): boolean {
  try {
    return window.localStorage.getItem(FIT_HOURS_STORAGE_KEY) === "1";
  } catch {
    return false;
  }
}

let cachedFitHours: boolean | null = null;
const fitHoursListeners = new Set<() => void>();

export function subscribeFitHours(onChange: () => void): () => void {
  fitHoursListeners.add(onChange);
  return () => {
    fitHoursListeners.delete(onChange);
  };
}

export function fitHoursSnapshot(): boolean {
  cachedFitHours ??= readStoredFitHours();
  return cachedFitHours;
}

/** Off on the server, for the reason `densityServerSnapshot` gives. */
export function fitHoursServerSnapshot(): boolean {
  return false;
}

/** Sets the crop for every subscriber, and remembers it. */
export function chooseFitHours(next: boolean): void {
  if (cachedFitHours === next) return;
  cachedFitHours = next;
  try {
    window.localStorage.setItem(FIT_HOURS_STORAGE_KEY, next ? "1" : "0");
  } catch {
    // Applies for this session; it just will not be remembered.
  }
  for (const listener of fitHoursListeners) listener();
}
